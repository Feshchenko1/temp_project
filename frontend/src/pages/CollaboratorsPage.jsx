...
/**
 * CollaboratorsPage - The Social Hub of Harmonix
 * Split layout: Left sidebar for chats/contacts, Right area for active collaboration.
 */
const CollaboratorsPage = () => {
  ...
  // Sorting logic within the render or a memo
  const sortedChats = [...recentChats].sort((a, b) => {
    // Sort strictly by recency (Telegram-style)
    // Preference: last message time, fallback to chat updatedAt
    const dateA = new Date(a.lastMessage?.createdAt || a.updatedAt);
    const dateB = new Date(b.lastMessage?.createdAt || b.updatedAt);
    return dateB - dateA;
  });

  // Derived state: Get the current active chat details
  ...
  useEffect(() => {
    if (chatId) {
      ...
      // Mark as read immediately if selected via URL
                ...
                .map((chat) => {
                  ...
                  return (
                    ...
                    <div className="relative">
                      ...
                      <div className={`absolute bottom-0 right-0 size-3 border-2 border-base-100 rounded-full transition-colors duration-300 ${
                        onlineUserIds.includes(chat.otherMember?.id) ? "bg-success" : "bg-base-content/20"
                      }`} />
                    </div>
